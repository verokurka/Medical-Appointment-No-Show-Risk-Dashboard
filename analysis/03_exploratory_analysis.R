...
# investigating sms further
sms_waiting_summary <- noshow %>%
  group_by(sms_status) %>%
  summarise(
    total_appointments = n(),
    avg_waiting_days = mean(waiting_days),
    median_waiting_days = median(waiting_days),
    no_show_percent = mean(no_show_flag) * 100,
    .groups = "drop"
  )

sms_waiting_summary
write_csv(sms_waiting_summary, "outputs/tables/sms_waiting_summary.csv")

# deeper check: no-show percent by waiting time and SMS status
sms_by_waiting_group <- noshow %>%
  group_by(waiting_days_group, sms_status) %>%
  summarise(
    total_appointments = n(),
    no_show_percent = mean(no_show_flag) * 100,
    .groups = "drop"
...
write_csv(sms_by_waiting_group, "outputs/tables/sms_by_waiting_group.csv")
...
# sms further investigation
sms_by_waiting_plot <- ggplot(
  sms_by_waiting_group,
  aes(x = waiting_days_group, y = no_show_percent, fill = sms_status)
) +
  geom_col(position = "dodge") +
  labs(
    title = "No-Show Percentage by Waiting Time and SMS Status",
    subtitle = "SMS reminder status should be interpreted alongside scheduling delay",
    x = "Waiting time between scheduling and appointment",
    y = "No-show percentage",
    fill = "SMS status"
  ) +
...
ggsave(
  filename = "outputs/charts/no_show_by_waiting_time_and_sms_status.png",
  plot = sms_by_waiting_plot,
  width = 9,
  height = 5
)
